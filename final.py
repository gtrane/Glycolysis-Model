...
def func(y, t, k1, kminus1, k2, kminus2, k3, kminus3, k4, kminus4, k5, kminus5, k6, kminus6, k7, kminus7, k8, kminus8,
        ...
        k50, kminus50, k51, kminus51, glc_in, glc_out, glc_ext, mgatp_in, mgatp_out, mgatp_ext, mgadp_in, mgadp_out,
        mgadp_ext, hplus_in, hplus_out, hplus_ext, hpo4_in, hpo4_out, hpo4_ext, nadplus_in, nadplus_out, nadplus_ext,
        nadh_in, nadh_out, nadh_ext, h2o_in, h2o_out, h2o_ext, pyr_in, pyr_out, pyr_ext):
    ...
    dydt = [
        ...
        # d(glc)/dt
        -k1 * hk * glc + kminus1 * hk_glc - k4 * glc * hk_mgatp + kminus4 * hk_mgatp_glc + glc_in * glc_ext
        - glc_out * glc,
        # d(hk_glc)/dt
        ...
        + k39 * pgk_mgatp - kminus39 * pgk * mgatp + k51 * pk_mgatp - kminus51 * pk * mgatp + mgatp_in * mgatp_ext
        - mgatp_out * mgatp,
        # d(hk_mgatp_glc)/dt
        ...
        - k49 * pep_pk_mgadp * hplus + kminus49 * pyr_pk_mgatp + hplus_in * hplus_ext - hplus_out * hplus,
        # d(hk_g6p)/dt
        ...
        + kminus48 * pep_pk_mgadp + mgadp_in * mgadp_ext - mgadp_out * mgadp,
        # d(g6p)/dt
        ...
        + nadplus_in * nadplus_ext - nadplus_out * nadplus,
        ...
        # d(hpo4)/dt
        -k30 * gapd_gapdh_nadplus * hpo4 + kminus30 * gapd_gapdh_nad_hpo4 + hpo4_in * hpo4_ext - hpo4_out * hpo4,
        ...
        # d(nadh)/dt
        k33 * gapdh_nadh - kminus33 * gapdh * nadh + nadh_in * nadh_ext - nadh_out * nadh,
        # d(pgk)/dt
        ...
        # d(h2o)/dt
        k43 * pg2_eno - kminus43 * pep_eno * h2o + h2o_in * h2o_ext - h2o_out * h2o,
        # d(pep)/dt
        ...
        # d(pyr_pk_mgatp)/dt
        k49 * pep_pk_mgadp * hplus - kminus49 * pyr_pk_mgatp - k50 * pyr_pk_mgatp + kminus51 * pyr * pk_mgatp,
        # d(pyr)/dt
        k50 * pyr_pk_mgatp - kminus50 * pyr * pk_mgatp + pyr_in * pyr_ext - pyr_out * pyr,
        # d(pk_mgatp)/dt
...
def all_steps_open(max_time=10000):
    """
    Full Model of Glycolysis
    ...
    #
    # rate constants for transport in/out
    #
    glc_in      = 0.005   # flow in first order rate constant (/sec)
    glc_out     = 0.005  # flow out first order rate constant (/sec)
    mgatp_in    = 0.05    # flow in first order rate constant (/sec)
    mgatp_out   = 0.05    # flow out first order rate constant (/sec)
    mgadp_in    = 0.05    # flow in first order rate constant (/sec)
    mgadp_out   = 0.05    # flow out first order rate constant (/sec)
    hplus_in    = 0.05    # flow in first order rate constant (/sec)
    hplus_out   = 0.05    # flow out first order rate constant (/sec)
    hpo4_in     = 0.05    # flow in first order rate constant (/sec)
    hpo4_out    = 0.05    # flow out first order rate constant (/sec)
    nadplus_in  = 0.005    # flow in first order rate constant (/sec)
    nadplus_out = 0.05    # flow out first order rate constant (/sec)
    nadh_in     = 0.05    # flow in first order rate constant (/sec)
    nadh_out    = 0.05    # flow out first order rate constant (/sec)
    h2o_in      = 0.05    # flow in first order rate constant (/sec)
    h2o_out     = 0.05    # flow out first order rate constant (/sec)
    pyr_in      = 0.005  # flow in first order rate constant (/sec)
    pyr_out     = 0.005    # flow out first order rate constant (/sec)
    #
    # constant exterior concentrations
    #
    glc_ext     = 0.5    # mM
    mgatp_ext   = 0.1     # mM
    mgadp_ext   = 1.0     # mM
    ...
    sol = odeint(func, y0, tspan, args=(k1, kminus1, k2, kminus2, k3, kminus3, k4, kminus4, k5, kminus5, k6, kminus6,
                                        ...
                                        k51, kminus51, glc_in, glc_out, glc_ext, mgatp_in, mgatp_out, mgatp_ext,
                                        mgadp_in, mgadp_out, mgadp_ext, hplus_in, hplus_out, hplus_ext, hpo4_in,
                                        hpo4_out, hpo4_ext, nadplus_in, nadplus_out, nadplus_ext, nadh_in, nadh_out,
                                        nadh_ext, h2o_in, h2o_out, h2o_ext, pyr_in, pyr_out, pyr_ext))
    plt.figure(1)
    plt.title('Free Enzymes - Open System')
    ...
    plt.figure(2)
    plt.title('Enzyme-Substrate Complexes #1 - Open System')
    ...
    plt.figure(3)
    plt.title('Enzyme-Substrate Complexes #2 - Open System')
    ...
    plt.figure(4)
    plt.title('Enzyme-Energy Carrier Complexes - Open System')
    ...
    plt.figure(5)
    plt.title('Ternary Complexes - Open System')
    ...
    plt.figure(6)
    plt.title('Quaternary Complexes - Open System')
    ...
    plt.figure(7)
    plt.title('Energy Carriers and Metabolites - Open System')
    ...
    plt.figure(8)
    plt.title('Product, Substrate and Intermediates - Open System')
    plt.plot(tspan, sol[:, 1], 'r', tspan, sol[:, 10], tspan, sol[:, 14], tspan, sol[:, 21], tspan, sol[:, 25],
